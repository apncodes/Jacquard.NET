...
A production-shaped customer service API. Each session gets its own isolated `Agent` with conversation history persisted to disk. Hooks log every tool call and model response. Built with ASP.NET Core minimal APIs and the `Jacquard.Extensions.DI` package.
